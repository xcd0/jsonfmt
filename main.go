...
func Json2Json() {
	...
	formattedData, err := json.MarshalIndent(tmp, "", "\t")
	if err != nil {
		return
	}
...
func FormatJson(args *Args, path string) (string, error) {
	...
	var tmp map[string]interface{}
	if err = json.Unmarshal(data, &tmp); err != nil {
		return "", err // Return the decode error.
	}

	// Format the JSON data.
	formattedData, err := json.MarshalIndent(tmp, "", "\t")
	if err != nil {
		return "", err // Return the encode error.
	}
	...
	if err := os.WriteFile(name, formattedData, 0644); err != nil {
